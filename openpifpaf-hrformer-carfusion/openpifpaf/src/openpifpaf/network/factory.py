...
BASE_FACTORIES = {
    ...
    # HRFormer configurations extracted and adapted from:
    # - https://github.com/HRNet/HRFormer/blob/1245b88b5824fbd8cdb358b5ee909a4e537a2ef5/cls/configs/hrt/hrt_base.yaml
    # - https://github.com/HRNet/HRFormer/blob/1245b88b5824fbd8cdb358b5ee909a4e537a2ef5/cls/configs/hrt/hrt_small.yaml
    # - https://github.com/HRNet/HRFormer/blob/1245b88b5824fbd8cdb358b5ee909a4e537a2ef5/cls/configs/hrt/hrt_tiny.yaml
    'hrformer_t': lambda: basenetworks.HRFormer(
        'hrformer_t', {'DROP_PATH_RATE': 0.0,
                   'STAGE1': {'NUM_MODULES': 1, 'NUM_BRANCHES': 1, 'NUM_BLOCKS': [2], 'NUM_CHANNELS': [64],
                              'NUM_HEADS': [2], 'NUM_MLP_RATIOS': [4], 'NUM_RESOLUTIONS': [[56, 56]],
                              'BLOCK': 'BOTTLENECK'},
                   'STAGE2': {'NUM_MODULES': 1, 'NUM_BRANCHES': 2, 'NUM_BLOCKS': [2, 2], 'NUM_CHANNELS': [18, 36],
                              'NUM_HEADS': [1, 2], 'NUM_MLP_RATIOS': [4, 4], 'NUM_RESOLUTIONS': [[56, 56], [28, 28]],
                              'NUM_WINDOW_SIZES': [7, 7],
                              'ATTN_TYPES': [[['isa_local', 'isa_local'], ['isa_local', 'isa_local']]],
                              'FFN_TYPES': [[['conv_mlp', 'conv_mlp'], ['conv_mlp', 'conv_mlp']]],
                              'BLOCK': 'TRANSFORMER_BLOCK'},
                   'STAGE3': {'NUM_MODULES': 3, 'NUM_BRANCHES': 3, 'NUM_BLOCKS': [2, 2, 2],
                              'NUM_CHANNELS': [18, 36, 72], 'NUM_HEADS': [1, 2, 4], 'NUM_MLP_RATIOS': [4, 4, 4],
                              'NUM_RESOLUTIONS': [[56, 56], [28, 28], [14, 14]], 'NUM_WINDOW_SIZES': [7, 7, 7],
                              'ATTN_TYPES': [
                              ...
                              'FFN_TYPES': [
                                  ...
                                  [['conv_mlp', 'conv_mlp'], ['conv_mlp', 'conv_mlp'], ['conv_mlp', 'conv_mlp']]],
                              'BLOCK': 'TRANSFORMER_BLOCK'},
                   'STAGE4': {'NUM_MODULES': 2, 'NUM_BRANCHES': 4, 'NUM_BLOCKS': [2, 2, 2, 2],
                              'NUM_CHANNELS': [18, 36, 72, 144], 'NUM_HEADS': [1, 2, 4, 8],
                              'NUM_MLP_RATIOS': [4, 4, 4, 4], 'NUM_RESOLUTIONS': [[56, 56], [28, 28], [14, 14], [7, 7]],
                              'NUM_WINDOW_SIZES': [7, 7, 7, 7], 'ATTN_TYPES': [
                           [['isa_local', 'isa_local'], ['isa_local', 'isa_local'], ['isa_local', 'isa_local'],
                            ['isa_local', 'isa_local']],
                           [['isa_local', 'isa_local'], ['isa_local', 'isa_local'], ['isa_local', 'isa_local'],
                            ['isa_local', 'isa_local']]],
                              'FFN_TYPES': [
                                  ...
                                  [['conv_mlp', 'conv_mlp'], ['conv_mlp', 'conv_mlp'], ['conv_mlp', 'conv_mlp'],
                                   ['conv_mlp', 'conv_mlp']]],
                              'BLOCK': 'TRANSFORMER_BLOCK'}}),
    'hrformer_s': lambda: basenetworks.HRFormer(
        'hrformer_s', {'DROP_PATH_RATE': 0.0,
                    'STAGE1': {'NUM_MODULES': 1, 'NUM_BRANCHES': 1, 'NUM_BLOCKS': [2], 'NUM_CHANNELS': [64],
                               'NUM_HEADS': [2], 'NUM_MLP_RATIOS': [4], 'NUM_RESOLUTIONS': [[56, 56]],
                               'BLOCK': 'BOTTLENECK'},
                    'STAGE2': {'NUM_MODULES': 1, 'NUM_BRANCHES': 2, 'NUM_BLOCKS': [2, 2], 'NUM_CHANNELS': [32, 64],
                               'NUM_HEADS': [1, 2], 'NUM_MLP_RATIOS': [4, 4], 'NUM_RESOLUTIONS': [[56, 56], [28, 28]],
                               'NUM_WINDOW_SIZES': [7, 7],
                               'ATTN_TYPES': [[['isa_local', 'isa_local'], ['isa_local', 'isa_local']]],
                               'FFN_TYPES': [[['conv_mlp', 'conv_mlp'], ['conv_mlp', 'conv_mlp']]],
                               'BLOCK': 'TRANSFORMER_BLOCK'},
                    'STAGE3': {'NUM_MODULES': 4, 'NUM_BRANCHES': 3, 'NUM_BLOCKS': [2, 2, 2],
                               'NUM_CHANNELS': [32, 64, 128], 'NUM_HEADS': [1, 2, 4], 'NUM_MLP_RATIOS': [4, 4, 4],
                               'NUM_RESOLUTIONS': [[56, 56], [28, 28], [14, 14]], 'NUM_WINDOW_SIZES': [7, 7, 7],
                               'ATTN_TYPES': [
                                   [['isa_local', 'isa_local'], ['isa_local', 'isa_local'], ['isa_local', 'isa_local']],
                                   [['isa_local', 'isa_local'], ['isa_local', 'isa_local'], ['isa_local', 'isa_local']],
                                   [['isa_local', 'isa_local'], ['isa_local', 'isa_local'], ['isa_local', 'isa_local']],
                                   [['isa_local', 'isa_local'], ['isa_local', 'isa_local'],
                                    ['isa_local', 'isa_local']]], 'FFN_TYPES': [
                            [['conv_mlp', 'conv_mlp'], ['conv_mlp', 'conv_mlp'], ['conv_mlp', 'conv_mlp']],
                            [['conv_mlp', 'conv_mlp'], ['conv_mlp', 'conv_mlp'], ['conv_mlp', 'conv_mlp']],
                            [['conv_mlp', 'conv_mlp'], ['conv_mlp', 'conv_mlp'], ['conv_mlp', 'conv_mlp']],
                            [['conv_mlp', 'conv_mlp'], ['conv_mlp', 'conv_mlp'], ['conv_mlp', 'conv_mlp']]],
                               'BLOCK': 'TRANSFORMER_BLOCK'},
                    'STAGE4': {'NUM_MODULES': 2, 'NUM_BRANCHES': 4, 'NUM_BLOCKS': [2, 2, 2, 2],
                               'NUM_CHANNELS': [32, 64, 128, 256], 'NUM_HEADS': [1, 2, 4, 8],
                            ...
                            [['conv_mlp', 'conv_mlp'], ['conv_mlp', 'conv_mlp'], ['conv_mlp', 'conv_mlp'],
                             ['conv_mlp', 'conv_mlp']],
                            [['conv_mlp', 'conv_mlp'], ['conv_mlp', 'conv_mlp'], ['conv_mlp', 'conv_mlp'],
                             ['conv_mlp', 'conv_mlp']]], 'BLOCK': 'TRANSFORMER_BLOCK'}}),
    'hrformer_b': lambda: basenetworks.HRFormer(
        'hrformer_b', {'DROP_PATH_RATE': 0.2,
                   'STAGE1': {'NUM_MODULES': 1, 'NUM_BRANCHES': 1, 'NUM_BLOCKS': [2], 'NUM_CHANNELS': [64],
                              'NUM_HEADS': [2], 'NUM_MLP_RATIOS': [4], 'NUM_RESOLUTIONS': [[56, 56]],
                              'BLOCK': 'BOTTLENECK'},
                   'STAGE2': {'NUM_MODULES': 1, 'NUM_BRANCHES': 2, 'NUM_BLOCKS': [2, 2], 'NUM_CHANNELS': [78, 156],
                              'NUM_HEADS': [2, 4], 'NUM_MLP_RATIOS': [4, 4], 'NUM_RESOLUTIONS': [[56, 56], [28, 28]],
                              'NUM_WINDOW_SIZES': [7, 7],
                              'ATTN_TYPES': [[['isa_local', 'isa_local'], ['isa_local', 'isa_local']]],
                              'FFN_TYPES': [[['conv_mlp', 'conv_mlp'], ['conv_mlp', 'conv_mlp']]],
                              'BLOCK': 'TRANSFORMER_BLOCK'},
                   'STAGE3': {'NUM_MODULES': 4, 'NUM_BRANCHES': 3, 'NUM_BLOCKS': [2, 2, 2],
                              'NUM_CHANNELS': [78, 156, 312], 'NUM_HEADS': [2, 4, 8], 'NUM_MLP_RATIOS': [4, 4, 4],
                              'NUM_RESOLUTIONS': [[56, 56], [28, 28], [14, 14]], 'NUM_WINDOW_SIZES': [7, 7, 7],
                              'ATTN_TYPES': [
                                  [['isa_local', 'isa_local'], ['isa_local', 'isa_local'], ['isa_local', 'isa_local']],
                                  [['isa_local', 'isa_local'], ['isa_local', 'isa_local'], ['isa_local', 'isa_local']],
                              ...
                              'BLOCK': 'TRANSFORMER_BLOCK'},
                   'STAGE4': {'NUM_MODULES': 2, 'NUM_BRANCHES': 4, 'NUM_BLOCKS': [2, 2, 2, 2],
                              'NUM_CHANNELS': [78, 156, 312, 624], 'NUM_HEADS': [2, 4, 8, 16],
                              'NUM_MLP_RATIOS': [4, 4, 4, 4], 'NUM_RESOLUTIONS': [[56, 56], [28, 28], [14, 14], [7, 7]],
                              'NUM_WINDOW_SIZES': [7, 7, 7, 7], 'ATTN_TYPES': [
                           [['isa_local', 'isa_local'], ['isa_local', 'isa_local'], ['isa_local', 'isa_local'],
                            ['isa_local', 'isa_local']],
