...
from .hrformer import HighResolutionTransformer
...
class HRFormer(BaseNetwork):
    ...
    def __init__(self, name, configuration):
        super().__init__(name, stride=32, out_features=1024 if self.remove_final_layer else 2048)

        self.backbone = HighResolutionTransformer(configuration, self.remove_final_layer)

        if self.pretrained:
            assert self.checkpoint is not None and self.checkpoint.strip() != ''\
                , 'Checkpoint needs to be set, if --hrformer-no-pretrain is *not* set'
            self.backbone.init_weights(self.checkpoint)
        else:
    ...
    def cli(cls, parser: argparse.ArgumentParser):
        ...
        group.add_argument('--hrformer-checkpoint',
                           default=None, type=str,
                           help='Path to checkpoint (must be set if --hrformer-no-pretrain is not set).')
        group.add_argument('--hrformer-remove-final-layer',
                           default=False, action='store_true',
                           help='Remove the final layer of the backbone')

    @classmethod
    def configure(cls, args: argparse.Namespace):
        cls.pretrained = args.hrformer_pretrained
        cls.checkpoint = args.hrformer_checkpoint
        cls.remove_final_layer = args.hrformer_remove_final_layer
